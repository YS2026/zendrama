...
const VIP_PLANS = [
  { id:1, name:"Неделя",   price:"99 ₽",   period:"7 дней",   coins:1050, bonus:630,  save:"",          popular:false },
  { id:2, name:"Месяц",    price:"299 ₽",  period:"30 дней",  coins:2100, bonus:1350, save:"Скидка 25%", popular:true  },
  { id:3, name:"3 месяца", price:"699 ₽",  period:"90 дней",  coins:5000, bonus:3000, save:"Скидка 35%", popular:false },
  { id:4, name:"Год",      price:"1990 ₽", period:"365 дней", coins:15000,bonus:10000,save:"Скидка 45%", popular:false },
];
...
const SUBTITLES_MAP = {
  14: Object.fromEntries(
    Array.from({length:116},(_,i)=>[i+1,{ru:`${SUBS_CDN}/okhota_ep${String(i+1).padStart(3,'0')}_ru.srt?v=3`}])
  )
};
...
function Badge({text}){const map={"Эксклюзив":{bg:"#1e1a40",color:C.accentLight},"Хит":{bg:"#3a1225",color:"#f472b6"},"Новинка":{bg:"#0d2e20",color:C.jade},"18+":{bg:"#2a1a2e",color:"#c084fc"}};const s=map[text]||{bg:"#1e1e2a",color:C.textMuted};return <span style={{background:s.bg,color:s.color,fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:4,letterSpacing:0.5}}>{text}</span>;}

function Player({series,episode,onClose,onNext,vip,unlockedEps}){
  ...
  return(
    <div onClick={touch} style={{position:'fixed',inset:0,background:'#000',zIndex:200,overflow:'hidden'}}>
      ...
      <div style={{position:'absolute',inset:0,zIndex:10,opacity:showCtrl?1:0,transition:'opacity 0.3s',pointerEvents:showCtrl?'auto':'none'}}>
        <div style={{position:'absolute',top:0,left:0,right:0,background:'linear-gradient(rgba(0,0,0,0.7),transparent)',padding:'16px',display:'flex',alignItems:'center',gap:12}}>
          <button onClick={e=>{e.stopPropagation();onClose();}} style={{...btn(40),flexShrink:0}}><IcoClose/></button>
          <span style={{color:'#fff',fontSize:13,fontWeight:600,flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{series.title} — Серия {episode}</span>
        </div>
...
function SeriesPage({series,onClose,vip,coins,setCoins,watchHistory,setWatchHistory}){
  const[tab,setTab]=useState('episodes'),[playerEp,setPlayerEp]=useState(null),[liked,setLiked]=useLS(`zd_like_${series.id}`,false),[notified,setNotified]=useLS(`zd_notify_${series.id}`,false);
  ...
  if(playerEp)return<Player series={series} episode={playerEp} onClose={()=>setPlayerEp(null)} vip={vip} unlockedEps={unlockedEps} onNext={()=>{const next=playerEp+1;if(next>series.episodes)return;if(next<=unlockedEps){setWatchHistory({...watchHistory,[series.id]:next});setPlayerEp(next);}else setPlayerEp(null);}}/>;
  return(
    <div style={{position:'fixed',inset:0,background:C.bg,zIndex:50,overflowY:'auto'}}>
      <div style={{position:'relative',height:320}}>
        ...
        <div style={{position:'absolute',inset:0,background:`linear-gradient(transparent 30%,${C.bg})`}}/>
        <div style={{position:'absolute',top:0,left:0,right:0,display:'flex',justifyContent:'space-between',padding:'16px'}}>
          <button onClick={onClose} style={{background:'rgba(0,0,0,0.5)',border:'none',color:'#fff',borderRadius:'50%',width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><IcoClose/></button>
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>setLiked(!liked)} style={{background:'rgba(0,0,0,0.5)',border:'none',color:liked?C.red:'#fff',borderRadius:'50%',width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><IcoHeart f={liked}/></button>
            <button onClick={()=>setNotified(!notified)} style={{background:'rgba(0,0,0,0.5)',border:'none',color:notified?C.gold:'#fff',borderRadius:'50%',width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><IcoBell/></button>
          </div>
        </div>
      </div>
      <div style={{padding:'0 16px 100px'}}>
        <div style={{display:'flex',gap:8,marginBottom:8,flexWrap:'wrap',alignItems:'center'}}><Badge text={series.badge}/><span style={{color:C.textMuted,fontSize:12}}>{series.genre}</span><span style={{color:C.gold,fontSize:12}}>★ {series.rating}</span><span style={{color:C.textDim,fontSize:12}}>· {series.episodes} серий</span></div>
        <h2 style={{color:C.text,fontSize:20,fontWeight:800,margin:'0 0 8px'}}>{series.title}</h2>
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:12}}>{(series.tags||[]).map(tag=><span key={tag} style={{background:C.card2,color:C.textMuted,fontSize:11,padding:'3px 10px',borderRadius:20,border:`1px solid ${C.textDim}`}}>{tag}</span>)}</div>
        <button onClick={()=>watch(lastWatched||1)} style={{width:'100%',background:C.accent,color:'#fff',border:'none',borderRadius:12,padding:'14px',fontSize:15,fontWeight:800,cursor:'pointer',marginBottom:16,display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>▶ {lastWatched>0?`Продолжить с серии ${lastWatched}`:'Смотреть'}</button>
        ...
        <div style={{display:'flex',borderBottom:`1px solid ${C.card2}`,marginBottom:4}}>{['episodes','info'].map(t=><button key={t} onClick={()=>setTab(t)} style={{flex:1,background:'none',border:'none',color:tab===t?C.accent:C.textMuted,borderBottom:tab===t?`2px solid ${C.accent}`:'2px solid transparent',padding:'10px',fontSize:14,cursor:'pointer',fontWeight:tab===t?700:400}}>{t==='episodes'?'Серии':'О сериале'}</button>)}</div>
        {tab==='episodes'&&<div>
          {!vip&&<div style={{background:`${C.accent}10`,border:`1px solid ${C.accent}25`,borderRadius:10,padding:'12px 14px',margin:'12px 0',display:'flex',alignItems:'center',gap:12}}><IcoCrown/><div style={{flex:1}}><div style={{color:C.gold,fontWeight:700,fontSize:13}}>ZenDrama Premium</div><div style={{color:C.textMuted,fontSize:11}}>Все серии · От 99 ₽/нед</div></div><span style={{color:C.accent,fontSize:12,fontWeight:700}}>→</span></div>}
          {Array.from({length:series.episodes},(_,i)=>i+1).map(ep=>{const locked=ep>unlockedEps;return(<div key={ep} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 8px',borderBottom:`1px solid ${C.card2}40`}}><div style={{width:40,height:40,borderRadius:8,background:locked?C.card2:`${C.accent}18`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{locked?<IcoLock/>:<span style={{color:C.accent,fontWeight:700,fontSize:13}}>{ep}</span>}</div><div style={{flex:1}}><div style={{color:locked?C.textMuted:C.text,fontSize:14}}>Серия {ep}</div><div style={{color:C.textDim,fontSize:11}}>~2 мин</div></div>{locked?<button onClick={()=>unlock(ep)} style={{background:C.accent,color:'#fff',border:'none',borderRadius:20,padding:'5px 14px',fontSize:12,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:4}}><CoinIco/> 5</button>:<button onClick={()=>watch(ep)} style={{background:'transparent',border:`1px solid ${C.accent}`,color:C.accent,borderRadius:20,padding:'5px 14px',fontSize:12,cursor:'pointer'}}>▶</button>}</div>);})}
        </div>}
        {tab==='info'&&<div style={{padding:'16px 0'}}><p style={{color:C.textMuted,fontSize:14,lineHeight:1.8,marginBottom:16}}>{series.desc}</p><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>{[['Жанр',series.genre],['Серий',series.episodes],['Рейтинг',`★ ${series.rating}`],['Доступно',`${unlockedEps} из ${series.episodes}`]].map(([k,v])=><div key={k} style={{background:C.card,borderRadius:10,padding:'12px'}}><div style={{color:C.textDim,fontSize:11,marginBottom:4}}>{k}</div><div style={{color:C.text,fontWeight:700,fontSize:14}}>{v}</div></div>)}</div></div>}
      </div>
    </div>
  );
}

function Shop({coins,setCoins,vip,setVip,onClose}){
  const[tab,setTab]=useState('vip');
  return(
    <div style={{position:'fixed',inset:0,background:C.bg,zIndex:60,overflowY:'auto'}}>
      <div style={{padding:'16px',display:'flex',alignItems:'center',gap:12}}><button onClick={onClose} style={{background:'none',border:'none',color:C.text,cursor:'pointer'}}><IcoClose/></button><span style={{color:C.text,fontWeight:800,fontSize:18}}>Магазин</span></div>
      <div style={{margin:'0 16px 16px',background:C.card,borderRadius:14,padding:16,display:'flex',alignItems:'center',gap:16}}><CoinIco size={36}/><div><div style={{color:C.textMuted,fontSize:12}}>Ваш баланс</div><div style={{color:C.gold,fontWeight:800,fontSize:24}}>{coins} монет</div></div><div style={{marginLeft:'auto',color:C.textMuted,fontSize:12}}>1 серия = 5 монет</div></div>
      <div style={{display:'flex',margin:'0 16px 20px',background:C.card,borderRadius:10,overflow:'hidden'}}>{['vip','coins','bonus'].map(t=><button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:'10px',background:tab===t?C.accent:'transparent',color:tab===t?'#fff':C.textMuted,border:'none',fontWeight:700,fontSize:13,cursor:'pointer'}}>{t==='vip'?'👑 Premium':t==='coins'?'🪙 Монеты':'🎁 Бонусы'}</button>)}</div>
      <div style={{padding:'0 16px 40px'}}>
        {tab==='vip'&&<><div style={{background:`${C.gold}12`,border:`1px solid ${C.gold}40`,borderRadius:16,padding:20,marginBottom:20}}><div style={{color:C.gold,fontSize:17,fontWeight:800,marginBottom:12}}>👑 ZenDrama Premium</div>{['Все серии без ограничений','Без рекламы','HD качество','Монеты каждый день'].map(f=><div key={f} style={{color:C.textMuted,fontSize:13,display:'flex',gap:8,marginBottom:6}}><span style={{color:C.jade}}>✓</span>{f}</div>)}</div>{VIP_PLANS.map(plan=><div key={plan.id} onClick={()=>{setVip(true);onClose();alert(`Premium "${plan.name}" подключён!`);}} style={{background:plan.popular?`${C.accent}12`:C.card,border:`${plan.popular?2:1}px solid ${plan.popular?C.accent:C.card2}`,borderRadius:12,padding:'16px',marginBottom:12,cursor:'pointer',position:'relative'}}>{plan.popular&&<span style={{position:'absolute',top:-10,left:'50%',transform:'translateX(-50%)',background:C.red,color:'#fff',fontSize:10,padding:'2px 10px',borderRadius:10,fontWeight:700}}>Популярно</span>}<div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{color:C.text,fontWeight:700,fontSize:15}}>{plan.name}</div><div style={{color:C.textMuted,fontSize:12}}>{plan.period} · +{plan.coins} монет</div>{plan.save&&<div style={{color:C.jade,fontSize:11,marginTop:2}}>{plan.save}</div>}</div><div style={{color:C.accent,fontWeight:800,fontSize:20}}>{plan.price}</div></div></div>)}</>}
        {tab==='coins'&&<>{COINS_PACKAGES.map(pkg=><div key={pkg.id} onClick={()=>{setCoins(coins+pkg.coins);alert(`+${pkg.coins} монет!`);}} style={{background:pkg.popular?`${C.accent}12`:C.card,border:`${pkg.popular?2:1}px solid ${pkg.popular?C.accent:C.card2}`,borderRadius:12,padding:'16px',marginBottom:12,cursor:'pointer',display:'flex',alignItems:'center'}}><CoinIco size={32}/><div style={{flex:1,marginLeft:12}}><div style={{color:C.text,fontWeight:700,fontSize:15}}>{pkg.coins} монет{pkg.popular&&<span style={{background:C.red,color:'#fff',fontSize:10,padding:'2px 7px',borderRadius:4,marginLeft:8}}>Выгодно</span>}</div>{pkg.bonus&&<div style={{color:C.jade,fontSize:12}}>{pkg.bonus}</div>}</div><div style={{color:C.accent,fontWeight:800,fontSize:18}}>{pkg.price}</div></div>)}</>}
        {tab==='bonus'&&<><div style={{background:C.card,borderRadius:12,padding:20,marginBottom:12,textAlign:'center'}}><div style={{fontSize:40,marginBottom:8}}>📅</div><div style={{color:C.text,fontWeight:700,fontSize:15,marginBottom:6}}>Ежедневный чекин</div><div style={{color:C.textMuted,fontSize:13,marginBottom:16}}>+10 монет каждый день</div><button onClick={()=>{setCoins(coins+10);alert('+10 монет!');}} style={{background:C.accent,color:'#fff',border:'none',borderRadius:10,padding:'12px 32px',fontSize:14,fontWeight:700,cursor:'pointer'}}>Получить 10 монет</button></div><div style={{background:C.card,borderRadius:12,padding:20,textAlign:'center'}}><div style={{fontSize:40,marginBottom:8}}>📺</div><div style={{color:C.text,fontWeight:700,fontSize:15,marginBottom:6}}>Смотри рекламу</div><div style={{color:C.textMuted,fontSize:13,marginBottom:16}}>+5 монет за просмотр</div><button onClick={()=>{setCoins(coins+5);alert('+5 монет!');}} style={{background:C.card2,color:C.accentLight,border:`1px solid ${C.accent}`,borderRadius:10,padding:'12px 32px',fontSize:14,fontWeight:700,cursor:'pointer'}}>Смотреть рекламу</button></div></>}
      </div>
    </div>
  );
}
...
function SeriesCard({series,onClick,watchHistory}){
  ...
  return(
    <div onClick={()=>onClick(series)} style={{cursor:'pointer',borderRadius:10,overflow:'hidden',background:C.card,transition:'transform 0.18s'}} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.03)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}>
...
function SearchPage({onSelectSeries,watchHistory}){
  ...
  return(
    <div style={{padding:'16px 16px 100px'}}>
      <div style={{position:'relative',marginBottom:16}}>
        ...
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Найти сериал..." autoFocus style={{width:'100%',background:C.card,border:`1px solid ${C.card2}`,borderRadius:10,padding:'11px 12px 11px 44px',color:C.text,fontSize:14,outline:'none',boxSizing:'border-box'}}/>
        {q&&<button onClick={()=>setQ('')} style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',color:C.textMuted,cursor:'pointer',fontSize:18}}>✕</button>}
      </div>
      <div style={{display:'flex',gap:8,overflowX:'auto',scrollbarWidth:'none',paddingBottom:12,marginBottom:8}}>
        {TAGS.map(tag=><button key={tag} onClick={()=>setActiveTag(activeTag===tag?null:tag)} style={{background:activeTag===tag?C.accent:`${C.accent}15`,color:activeTag===tag?'#fff':C.accentLight,border:`1px solid ${activeTag===tag?C.accent:C.accent+'30'}`,borderRadius:20,padding:'6px 14px',fontSize:12,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>{tag}</button>)}
      </div>
      {(q||activeTag)?<><div style={{color:C.textMuted,fontSize:12,marginBottom:12}}>Найдено: {results.length}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>{results.map(s=><SeriesCard key={s.id} series={s} onClick={onSelectSeries} watchHistory={watchHistory}/>)}</div></>:<><div style={{color:C.text,fontSize:14,fontWeight:700,marginBottom:12}}>🔥 В тренде</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>{SERIES_DATA.filter(s=>s.trending).sort((a,b)=>a.trending-b.trending).map(s=><SeriesCard key={s.id} series={s} onClick={onSelectSeries} watchHistory={watchHistory}/>)}</div></>}
    </div>
  );
}
...
export default function App(){
  ...
  const filtered=SERIES_DATA.filter(s=>genre==='Все'||s.genre===genre),trending=[...SERIES_DATA].filter(s=>s.trending).sort((a,b)=>a.trending-b.trending),continueList=SERIES_DATA.filter(s=>Object.keys(history).map(Number).includes(s.id)),featured=SERIES_DATA[0];
  return(
    <div style={{background:C.bg,minHeight:'100vh',maxWidth:430,margin:'0 auto',fontFamily:'system-ui,sans-serif',paddingBottom:70,color:C.text}}>
      {selected&&<SeriesPage series={selected} onClose={()=>setSelected(null)} vip={vip} coins={coins} setCoins={setCoins} watchHistory={history} setWatchHistory={setHistory}/>}
      {showShop&&<Shop coins={coins} setCoins={setCoins} vip={vip} setVip={setVip} onClose={()=>setShowShop(false)}/>}
      {tab==='home'&&<div>
        <div style={{padding:'14px 16px 10px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <ZenLogo size={26}/>
          <div style={{display:'flex',gap:10,alignItems:'center'}}>
            {vip&&<div style={{color:C.gold,fontSize:11,fontWeight:700,background:`${C.gold}15`,borderRadius:20,padding:'4px 10px',border:`1px solid ${C.gold}35`}}>👑 Premium</div>}
        ...
        <div onClick={()=>setSelected(featured)} style={{margin:'0 16px 20px',borderRadius:16,overflow:'hidden',cursor:'pointer',position:'relative',height:200}}>
        ...
        {continueList.length>0&&<><div style={{padding:'0 16px 10px',color:C.text,fontSize:15,fontWeight:700}}>▶ Продолжить</div><div style={{display:'flex',gap:10,padding:'0 16px 20px',overflowX:'auto',scrollbarWidth:'none'}}>{continueList.map(s=>{const ep=history[s.id]||0,pct=Math.round((ep/s.episodes)*100);return(<div key={s.id} onClick={()=>setSelected(s)} style={{flexShrink:0,width:120,cursor:'pointer'}}><div style={{position:'relative',borderRadius:10,overflow:'hidden',aspectRatio:'2/3'}}><img src={s.cover} alt={s.title} style={{width:'100%',height:'100%',objectFit:'cover'}}/><div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(transparent,rgba(8,9,13,0.95))',padding:'18px 6px 6px'}}><div style={{color:C.text,fontSize:10,fontWeight:700}}>{s.title}</div><div style={{color:C.accent,fontSize:10}}>Сер. {ep}/{s.episodes}</div></div><div style={{position:'absolute',bottom:0,left:0,right:0,height:3,background:'#1e1e2a'}}><div style={{height:'100%',width:`${pct}%`,background:C.accent}}/></div></div></div>);})}</div></>}
        {genre==='Все'&&<><div style={{padding:'0 16px 10px',color:C.text,fontSize:15,fontWeight:700}}>🔥 В тренде</div><div style={{display:'flex',gap:10,padding:'0 16px 20px',overflowX:'auto',scrollbarWidth:'none'}}>{trending.map(s=><div key={s.id} onClick={()=>setSelected(s)} style={{flexShrink:0,width:120,cursor:'pointer'}}><div style={{position:'relative',borderRadius:10,overflow:'hidden',aspectRatio:'2/3'}}><img src={s.cover} alt={s.title} style={{width:'100%',height:'100%',objectFit:'cover'}}/><div style={{position:'absolute',top:6,left:6,width:22,height:22,background:C.accent,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:900,fontSize:11}}>{s.trending}</div><div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(transparent,rgba(8,9,13,0.95))',padding:'18px 6px 6px'}}><div style={{color:C.text,fontSize:11,fontWeight:700}}>{s.title}</div></div></div></div>)}</div></>}
        <div style={{padding:'0 16px 10px',color:C.text,fontSize:15,fontWeight:700}}>⭐ Все сериалы</div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,padding:'0 16px'}}>{filtered.map(s=><SeriesCard key={s.id} series={s} onClick={setSelected} watchHistory={history}/>)}</div>
      </div>}
      {tab==='search'&&<SearchPage onSelectSeries={setSelected} watchHistory={history}/>}
      {tab==='history'&&<div style={{padding:'16px 16px 100px'}}><h2 style={{color:C.text,fontSize:18,fontWeight:800,margin:'0 0 16px'}}>История</h2>{continueList.length===0?<div style={{textAlign:'center',color:C.textMuted,marginTop:80}}><div style={{fontSize:48,marginBottom:12}}>📺</div><div>История просмотров пуста</div></div>:continueList.map(s=>{const ep=history[s.id]||0,pct=Math.round((ep/s.episodes)*100);return(<div key={s.id} onClick={()=>setSelected(s)} style={{display:'flex',gap:12,marginBottom:14,cursor:'pointer',background:C.card,borderRadius:10,overflow:'hidden'}}><div style={{position:'relative',width:90,flexShrink:0}}><img src={s.cover} alt={s.title} style={{width:90,height:130,objectFit:'cover',display:'block'}}/><div style={{position:'absolute',bottom:0,left:0,right:0,height:3,background:'#1e1e2a'}}><div style={{height:'100%',width:`${pct}%`,background:C.accent}}/></div></div><div style={{padding:'12px 12px 12px 0',flex:1}}><div style={{color:C.text,fontWeight:700,fontSize:14,marginBottom:4}}>{s.title}</div><div style={{color:C.accent,fontSize:12}}>Серия {ep} из {s.episodes}</div><div style={{color:C.textMuted,fontSize:11,marginTop:4}}>{s.genre}</div></div></div>);})}}</div>}
      {tab==='profile'&&<div style={{padding:'16px 16px 100px'}}><h2 style={{color:C.text,fontSize:18,fontWeight:800,margin:'0 0 20px'}}>Профиль</h2>{!vip?<div onClick={()=>setShowShop(true)} style={{background:`${C.gold}10`,border:`1px solid ${C.gold}30`,borderRadius:14,padding:16,marginBottom:16,cursor:'pointer'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{color:C.gold,fontWeight:800,fontSize:15}}>👑 ZenDrama Premium</div><div style={{color:C.textMuted,fontSize:12,marginTop:2}}>Безлимитный доступ</div></div><div style={{background:C.accent,color:'#fff',borderRadius:20,padding:'6px 14px',fontSize:12,fontWeight:700}}>Подключить</div></div></div>:<div style={{background:`${C.gold}15`,border:`2px solid ${C.gold}`,borderRadius:14,padding:16,marginBottom:16}}><div style={{color:C.gold,fontWeight:800,fontSize:15}}>👑 Premium активен</div></div>}<div style={{background:C.card,borderRadius:14,padding:16,marginBottom:16}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}><div style={{color:C.text,fontWeight:700,fontSize:15}}>Мой кошелёк</div><button onClick={()=>setShowShop(true)} style={{background:C.accent,color:'#fff',border:'none',borderRadius:20,padding:'5px 14px',fontSize:12,fontWeight:700,cursor:'pointer'}}>Пополнить</button></div><div style={{display:'flex',gap:24}}><div><div style={{color:C.gold,fontSize:24,fontWeight:800,display:'flex',alignItems:'center',gap:6}}><CoinIco size={20}/>{coins}</div><div style={{color:C.textMuted,fontSize:12}}>Монеты</div></div></div></div>{[{icon:<IcoHistory/>,label:'История просмотров',count:continueList.length},{icon:<IcoHeart f={false}/>,label:'Избранное'},{icon:<IcoGift/>,label:'Бонусный центр',action:()=>setShowShop(true)}].map(item=><div key={item.label} onClick={item.action} style={{background:C.card,borderRadius:12,padding:'14px 16px',marginBottom:10,display:'flex',alignItems:'center',gap:12,cursor:'pointer'}}><div style={{color:C.textMuted}}>{item.icon}</div><div style={{flex:1,color:C.text,fontSize:14}}>{item.label}</div>{item.count!==undefined&&<span style={{background:C.accent,color:'#fff',borderRadius:20,padding:'2px 8px',fontSize:11,fontWeight:700}}>{item.count}</span>}<span style={{color:C.textDim}}>›</span></div>)}</div>}
